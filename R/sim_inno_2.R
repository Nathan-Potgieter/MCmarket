...
data <- map_dfc(1:10, ~runif(100)) %>% purrr::set_names(LETTERS[1:10])
mnsd_df <- data.frame(Type = LETTERS[1:10], Mn = 1:10, SD = 1:10)

# Now, make things tidy and everything is very very very easy:
data %>% gather(Type, Value) %>% left_join(., mnsd_df, by = "Type") %>%
    group_by(Type) %>%
    mutate(QN = qnorm(Value, Mn, SD) ) %>%
    # Optional:
    # To allow spreading, if you want to go back to wide data (but why would you though?????????!!!!!):
    mutate(Rowname = row_number()) %>%
    ungroup() %>%
    select(Type, QN, Rowname) %>% spread(Type, QN) %>% select(-Rowname)
