...
This document walks through the typical workflow, and highlights some key considerations, when performing the Monte Carlo simulation of financial asset markets.  
...
Before performing any simulations one must first think about what kind of market it is that they want to simulate. This depends on the purpose for carrrying out Monte Carlo simulations. Since the correlation matrix is the key input when simulating markets in MCmarket, lets start by looking as some. Users can estimate their own correlation matrices with real world date, or create _ad hoc_ matrices to simulate markets with specific dependence structures.

### ad hoc/ constructed correlation matricies

- The function _gen_corr_ allows users to easily generate _ad hoc_ correlation matrices with a set number of clusters and up to 4 layers.
...
If the _ad hoc_ matrices don't seem realistic enough, or the exact correlation structure is not of particular interest, then one can use empirical data to estimate a correlation matrix of their own. Alternatively, _MCmarket_ comes with a data set of 30 correlation matrices that were each estimated from a random subset of 50 S&P500 stocks over a random time slice of 252 days between 1 January 2000 and 1 January 2020. 

These correlation matrices are labeled  as ‘stressed’ if they were estimated on a market where an equi-weighted basket of stocks had a Sharpe ratio below -0.5, ‘rally’ if the equi-weighted basket of stocks had a Sharpe ratio above 2 and ‘normal’ if the equi-weighted basket of stocks had a Sharpe in-between -0.5 and 2. See _MCmarket::corr_mats_
